...
import { Card, Title, Paragraph, Button as PaperButton, Chip, Surface, IconButton } from 'react-native-paper';
...
const Event = ({ navigation, search = '', eventType = '' }) => {
  ...
  const filteredEvents = events.filter(event => {
    ...
    const matchSearch = nameToCheck.toLowerCase().includes(search.toLowerCase());
    const matchType = eventType ? (event.type === eventType || event.category?.name === eventType) : true;
  ...
  if (error && !refreshing) {
    return (
      <View style={styles.errorContainer}>
        <MaterialCommunityIcons name="calendar-alert" size={60} color={COLORS.error} />
