...
const Home = ({ navigation }) => {
  ...
  const isValidUser =
    ...
    (user.role !== undefined ||
      ...
      user.id !== undefined);
  const isOrganizer =
    isValidUser && (user.role === "organizer" || user.role === "admin");
      ...
      <Chip
        ...
        icon={(props) => (
          <MaterialCommunityIcons
            ...
            size={16}
            color={
              eventStatus === status.value ? COLORS.onPrimary : COLORS.primary
            }
          />
        )}
      >
    ...
    (event) => {
      ...
      return (
        ...
        >
          ...
          >
            <Card.Cover source={{ uri: imageUri }} style={styles.eventImage} />
      ...
      return (
        <View style={styles.loadingContainer}>
          ...
          <Text style={{ color: COLORS.textSecondary, marginTop: 8 }}>
            Đang tải sự kiện{" "}
            {eventStatus
              ? `${eventStatusOptions.find((s) => s.value === eventStatus)?.label}`
              : ""}
            ...
          </Text>
        </View>
        ...
        <View style={styles.eventsContainer}>
          <View style={styles.categoryHeaderContainer}>
            <Text style={styles.sectionHeader}>
              {"Sự kiện " +
                eventStatusOptions.find((s) => s.value === eventStatus)
                  ?.label || "Sự kiện"}
            </Text>
            <TouchableOpacity onPress={showAllEvents}>
              <Text style={styles.categorySeeAll}>Xem tất cả</Text>
